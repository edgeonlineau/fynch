...
describe('isFormDuplicate', () => {
  ...
  describe('Tier 1: lead_id lifetime dedup', () => {
    it('suppresses a second fire with the same (provider, lead_id) shortly after', () => {
      const params = { service_provider: 'hubspot-v3', form_id: 'f1', lead_id: 'abc' };
    ...
    it('allows different lead_ids under the same provider', () => {
      expect(
        isFormDuplicate({ service_provider: 'hubspot-v3', form_id: 'f1', lead_id: 'abc' }),
      ).toBe(false);
      expect(
        isFormDuplicate({ service_provider: 'hubspot-v3', form_id: 'f1', lead_id: 'def' }),
      ).toBe(false);
    });

    it('allows the same lead_id value under different providers', () => {
      expect(isFormDuplicate({ service_provider: 'hubspot-v3', lead_id: 'shared-id' })).toBe(false);
      expect(isFormDuplicate({ service_provider: 'typeform', lead_id: 'shared-id' })).toBe(false);
    });
  ...
  describe('Tier 2: form-identity windowed dedup', () => {
    it('suppresses repeat fire of same form within the 2s window', () => {
      const params = { service_provider: 'elementor', form_id: '12', form_name: 'Contact' };
      expect(isFormDuplicate(params)).toBe(false);
    ...
    it('over-suppresses providers that emit no identity (duda)', () => {
      const params = { service_provider: 'duda' };
      expect(isFormDuplicate(params)).toBe(false);
    ...
    it('treats empty string and undefined form_name as the same key', () => {
      expect(isFormDuplicate({ service_provider: 'elementor', form_id: '12', form_name: '' })).toBe(
        false,
      );
      expect(
        isFormDuplicate({ service_provider: 'elementor', form_id: '12', form_name: undefined }),
      ).toBe(true);
    });

    it('treats empty string and undefined form_id as the same key', () => {
      expect(isFormDuplicate({ service_provider: 'duda', form_id: '' })).toBe(false);
      expect(isFormDuplicate({ service_provider: 'duda', form_id: undefined })).toBe(true);
    });

    it('keeps different forms under same provider independent', () => {
      expect(isFormDuplicate({ service_provider: 'elementor', form_id: '12' })).toBe(false);
      expect(isFormDuplicate({ service_provider: 'elementor', form_id: '99' })).toBe(false);
    });
  });
  ...
  describe('memory bounds', () => {
    it('caps Tier 1 lead-id set at 500 entries with oldest evicted', () => {
      for (let i = 0; i < 600; i++) {
        isFormDuplicate({ service_provider: 'hubspot-v3', lead_id: `lead-${i}` });
      }
      expect(testing.sizes().leadIds).toBeLessThanOrEqual(500);

      expect(isFormDuplicate({ service_provider: 'hubspot-v3', lead_id: 'lead-0' })).toBe(false);
      expect(isFormDuplicate({ service_provider: 'hubspot-v3', lead_id: 'lead-599' })).toBe(true);
    });

    it('caps Tier 2 form-key map at 100 entries', () => {
      for (let i = 0; i < 150; i++) {
        isFormDuplicate({ service_provider: 'elementor', form_id: `form-${i}` });
      }
      expect(testing.sizes().formKeys).toBeLessThanOrEqual(100);
    });

    it('prunes Tier 2 entries older than 2 * window on each call', () => {
      isFormDuplicate({ service_provider: 'elementor', form_id: 'old' });
      expect(testing.sizes().formKeys).toBe(1);

      vi.advanceTimersByTime(5000);
      isFormDuplicate({ service_provider: 'elementor', form_id: 'fresh' });
